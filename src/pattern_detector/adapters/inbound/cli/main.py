"""Typer and Rich CLI interface for DPX-Kotlin."""
...
app = typer.Typer(
    name="dpx-kotlin",
    help="🎯 Modern Kotlin Static Analyzer: Coroutines, Flow, Jetpack Compose, GoF & Multiplatform Architectural Pattern Detector.",
...
def scan(
    path: Annotated[
        str,
        typer.Argument(
            help="Path to a Kotlin source file (.kt/.kts) or directory containing a Gradle/Maven Kotlin project.",
        ),
    ] = ".",
    ...
    pattern: Annotated[
        ...
        typer.Option(
            "--pattern",
            "-p",
            help="Filter for specific pattern type(s) (e.g. 'structured_concurrency_scope', 'sealed_hierarchy_adt', 'globalscope_coroutine_leak').",
        ),
    ] = None,
    ...
    exclude: Annotated[
        ...
        typer.Option(
            "--exclude",
            "-e",
            help="Directory name(s) to exclude from scanning (e.g. -e build -e test).",
        ),
    ] = None,
    verbose: Annotated[
        bool,
    ...
    """Scan a Kotlin codebase for Coroutines, Flow, Jetpack Compose, GoF patterns, and safety hazards."""
...
@app.command()
def rules() -> None:
    """List all available Kotlin detection rules and architectural checks."""
    table = Table(
        title="🎯 DPX-Kotlin: Supported Pattern Rules & Quality Checks",
...
def info(
    pattern_name: Annotated[
        str,
        typer.Argument(
            help="Pattern type or name to query (e.g. 'structured_concurrency_scope', 'sealed_hierarchy_adt').",
        ),
    ],
) -> None:
    """Display deep architectural details, rationale, and advice for a specific Kotlin pattern."""
    ...
    panel = Panel.fit(
        ...
        f"[bold white]Kotlin Target:[/bold white] {pdef.kotlin_version}\n\n"
        f"[bold white]Description:[/bold white]\n{pdef.description}\n\n"
        f"[bold white]Architecture Recommendation:[/bold white]\n{pdef.recommendation or 'Adheres to standard Kotlin idiom.'}",
        title=f"🔍 Kotlin Architecture Encyclopedia: {pdef.name}",
